              ...
              <Headerx />
                ...
                <FlexboxLayout class="dashBoardAtas">
                  <FlexboxLayout class="DashboardDivItem">
                    ...
                    <Label class="DashboardDivItemLabel1">Isi</Label>
                    <Label class="DashboardDivItemLabel2">Saldo</Label>
                  </FlexboxLayout>
                </FlexboxLayout>
                        ...
                        <StackLayout width="100%">
                          ...
                          <FlexboxLayout v-if="loadingData ==false" class="flexListData" v-for="data in listData" :key="data.id">
                            <FlexboxLayout class="flexListDataItem">
                                <Label class="flexKetQRLabel">Rp. {{data.jumlah_topup}} ({{UMUM.tglConvert(data.createAt).tgl}} || {{UMUM.tglConvert(data.createAt).time}} WITA)</Label>
                                <Label class="flexKetQRLabel">{{data.master_unit_uraian}}</Label>
                                <Label class="labelNotifGreen">Sudah Terbayarkan</Label>
                            </FlexboxLayout>
  ...
  export default {
    ...
    methods: {
        ...
        getView: function () {
          ...
          fetch(this.$store.state.url.CLIENT_RIWAYAT_SALDO + "view", {
              method: "POST",
              headers: {
        ...
        getViewOne: function () {
          ...
          fetch(this.$store.state.url.CLIENT_RIWAYAT_SALDO + "viewOne", {
              method: "POST",
              headers: {
                  "content-type": "application/json",
                  authorization: "kikensbatara " + AppSettings.getString("token")
